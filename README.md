...
- 제목: `[TYPE] 작업 내용` — **TYPE은 작업 브랜치 접두사를 따릅니다.** 브랜치와 PR 제목이 항상 일치합니다.

  | 브랜치 접두사 | PR 제목 |
  | --- | --- |
  | `feature/` | `[FEAT] 공고 목록 페이지 구현` |
  | `fix/` | `[FIX] 북마크 토글 롤백 오류 수정` |
  | `refactor/` | `[REFACTOR] api 클라이언트 분리` |
  | `chore/` | `[CHORE] PR 작성 규칙 정리` |
  | `docs/` | `[DOCS] FSD 레이어 가이드 추가` |
  | `hotfix/` | `[HOTFIX] 로그인 500 오류 긴급 수정` |

  `feature/`는 `[FEATURE]`가 아니라 커밋 Type과 같은 `[FEAT]`를 씁니다. 도메인은 제목 태그가 아니라 설명과 `area:` 라벨로 표현합니다.

- 본문에 관련 Issue, 작업 내용, 확인이 필요한 부분을 적습니다.
- **본문은 60줄 안쪽으로 유지합니다.** 요약 2~3줄, 작업 내용은 한 줄짜리 8개 이내, 리뷰 참고 사항은 3개 이내로 적고, 명령 출력·표·제외한 작업처럼 긴 내용은 `<details>`로 접습니다. 커밋 목록은 GitHub이 이미 보여주므로 옮겨 적지 않습니다. 길게 써야만 설명되는 PR이라면 대개 한 PR에 너무 많이 담은 것입니다.
