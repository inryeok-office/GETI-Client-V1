...
describe('CompanyDetail', () => {
  it('기업 정보와 채용 공고를 표시한다', () => {
    render(
      <CompanyDetail
        ...
        company={COMPANY}
        jobs={JOBS}
        retryHref="/companies/company-1"
      />,
    );

    expect(screen.getAllByText('네이버클라우드').length).toBeGreaterThan(0);
    expect(screen.getByText('대기업 · IT 서비스')).toBeInTheDocument();
  ...
  it('로딩 상태에서 스켈레톤을 표시한다', () => {
    render(
      <CompanyDetail status="loading" company={null} jobs={[]} retryHref="/companies/company-1" />,
    );

    expect(screen.getByRole('status', { name: '기업 정보를 불러오는 중' })).toBeInTheDocument();
  });

  it('에러 상태에서 다시 시도 링크를 표시한다', () => {
    render(
      <CompanyDetail status="error" company={null} jobs={[]} retryHref="/companies/company-1" />,
    );
  ...
  it('비공개 · 삭제된 기업이면 접근 불가 안내를 표시한다', () => {
    render(
      <CompanyDetail
        status="success"
        company={{ ...COMPANY, unavailableReason: '삭제' }}
        jobs={[]}
        retryHref="/companies/company-1"
      />,
    );
