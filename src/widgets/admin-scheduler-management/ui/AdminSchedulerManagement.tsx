...
interface AdminSchedulerManagementProps {
  listStatus: AdminSchedulerListStatus;
  onRetry: () => void;
  tasks: OperationJob[];
}
        ...
        <div className="w-full max-w-[1620px]">
          ...
          <section className="mt-6" aria-labelledby="scheduler-list-heading">
            ...
            >
              ...
              {listStatus === 'error' ? <SchedulerError onRetry={onRetry} /> : null}
              {listStatus === 'empty' ? <SchedulerEmpty /> : null}
              {listStatus === 'success' ? <SchedulerTable tasks={tasks} /> : null}
            </div>
          </section>
...
function SchedulerTable({ tasks }: SchedulerTableProps) {
  return (
    <table className="w-full min-w-[1530px] table-fixed border-collapse text-left">
      ...
      <thead className="bg-neutral-50">
        <tr className="h-[66px]">
          {['작업', '실행 주기', '최근 실행', '다음 실행', '상태', '수동 실행'].map(
            (label, index) => (
              <th
                key={`${label}-${index}`}
                scope="col"
      ...
      <tbody>
        {tasks.map((task) => (
          <tr key={task.taskId} className="h-[66px] bg-white">
            ...
            <td className="px-5">
              <SchedulerAction actionStatus={task.actionStatus} />
            </td>
          </tr>
        ))}
      </tbody>
...
function SchedulerAction({ actionStatus }: Pick<OperationJob, 'actionStatus'>) {
  return (
    <span
      className={
