    ...
    <div>
      ...
      <div className="mt-[20px] flex flex-wrap items-center gap-[8px]">
        {FILTERS.map((filter) => {
          ...
          const isSource = filter.key === 'source';
          ...
          return (
            ...
            >
              <button
                ...
                onClick={() => {
                  if (isSourceError) {
                    jobSourcesQuery.refetch();
                    return;
                  }
                  setOpenFilter((prev) => (prev === filter.key ? null : filter.key));
                }}
                disabled={isFilterDisabled}
                className={`flex w-[168px] items-center justify-between rounded-[8px] border py-[16px] pr-[8px] pl-[16px] text-[14px] leading-[1.4] font-medium tracking-[-0.14px] disabled:cursor-not-allowed disabled:opacity-50 ${
                  isSourceError
              ...
              >
                ...
                <span className="flex h-[10px] w-[20px] shrink-0 items-center justify-center">
                  <Icon
                    name={isSourceError ? 'refresh' : 'chevronRight'}
                    className={
                      isSourceError
                        ? 'text-status-error size-[14px]'
                        : 'h-[20px] w-[10px] rotate-90 text-[#525252]'
                    }
                  />
                </span>
              </button>
