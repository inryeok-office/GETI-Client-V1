 ...
 * 없어 답변에 포함되지 않는다. `uploadError`도 `fileId`도 없는 항목은 업로드 응답을 기다리는
 * 중이라 "업로드 중" 표시를 보여준다 — 완료 전까지는 `fileId`가 없어 이 항목도 답변에 포함되지
 * 않으므로, 업로드가 끝나기 전에 저장 · 제출하면 안 된다(`JobApplyPage`가 막는다).
  ...
  return (
    <section className="flex w-full flex-col gap-[16px] rounded-[16px] bg-white p-[32px]">
      ...
      {attachments.map((file) => {
        const isUploading = file.uploadError === null && file.fileId === null;
          ...
          >
            ...
            {file.uploadError ? (
              ...
              <div className="flex shrink-0 items-center gap-[12px]">
                <Icon name="spinner" className="size-[16px] animate-spin text-[#525252]" />
                <span className="text-[14px] leading-[1.5] tracking-[-0.14px] text-[#525252]">
                  업로드 중
                </span>
                <button
                  type="button"
