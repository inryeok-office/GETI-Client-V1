  ...
  return (
    <div className="space-y-1.5">
      ...
      <select
        ...
        className={`focus:border-primary-300 h-12 w-full rounded-lg border bg-white px-3 text-sm leading-[1.5] tracking-[-0.14px] text-neutral-900 outline-none disabled:cursor-not-allowed disabled:border-neutral-200 disabled:bg-neutral-100 ${errorMessage ? 'border-status-error focus:border-status-error' : 'border-neutral-200'} ${className}`}
      >
        {children}
      </select>
      {errorMessage ? (
