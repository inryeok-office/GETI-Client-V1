...
- 허용 Type은 `AGENTS.md`에 명시된 12종(`feat`, `fix`, `refactor`, `style`, `test`, `docs`, `chore`, `config`, `build`, `ci`, `perf`, `revert`)을 사용한다.
- `style`은 포맷팅·클래스 정리처럼 동작에 영향 없는 수정에 사용한다. **화면이 실제로 달라지는 UI 변경은 `style`이 아니라 `feat` 또는 `fix`다.** 이 구분을 자주 틀리므로 Commit 전에 확인한다.
...
- 락파일(`package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`) 변경은 의존성 변경(`build`)과 함께 Commit한다. 기능 Commit에 끼워 넣지 않는다.
- `git add .`을 습관적으로 사용하지 않는다. 빌드 산출물(`.next/`)이나 사용자의 다른 작업이 함께 Stage될 수 있다.
- 한글 Commit 메시지를 작성할 때 Windows 셸에서 인코딩이 깨질 수 있다. 필요하면 UTF-8 파일에 메시지를 작성하고 `git commit -F <파일>`을 사용한다.
...
- 선행 PR이 아직 Merge되지 않은 상태에서 그 위에 이어서 작업해야 한다면, Base를 해당 선행 Branch로 두는 Stacked PR을 사용하고 PR 본문에 Merge 순서를 명시한다.
- 작업이 진행 중이거나 리뷰를 미리 받고 싶은 단계에서는 Draft PR을 사용한다. 구현과 검증이 끝나 리뷰를 받을 준비가 되면 Ready for Review로 전환한다.
- PR 제목은 저장소 Convention `[도메인] 작업 내용`을 따르고, 본문에 `Closes #{issue-number}`로 연결한다.
- **UI 변경이 있으면 스크린샷/GIF가 필요하다.** AI는 이미지를 직접 첨부할 수 없으므로, 해당 자리를 비워 두고 사용자에게 첨부를 요청한다. 첨부한 것처럼 보고하지 않는다.
...
- Label 이름을 추측하지 않는다. `gh label list`로 실제 이름(이모지 포함)을 확인하고 정확히 일치하는 문자열을 사용한다.
...
- 미커밋 변경이 있는데 출처를 알 수 없으면 Stash하거나 삭제하지 않고 사용자에게 먼저 확인한다.
